...
void ui_update_params(UIState *s) {
  ...
  std::string brightness_setting = params.get("BrightnessSetting");
  if(brightness_setting == "0")
  {
    s->scene.brightness_preference = DIM;
  }
  else if(brightness_setting == "2")
  {
    s->scene.brightness_preference = BRIGHT;
  }
  else
  {
    s->scene.brightness_preference = AUTO;
  }
}
...
void Device::updateBrightness(const UIState &s) {
  ...
  switch (s.scene.brightness_preference) {
    case BRIGHT:
        brightness = 100;
        break;
    case DIM:
